...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	...
	Fan.on('credentials-updated', () => {
		fs.writeFileSync('./database/options/sb.json', JSON.stringify(Fan.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'MICHAEL  LOADING...')
	})
	fs.existsSync('./database/options/sb.json') && Fan.loadAuthInfo('./database/options/sb.json')
	Fan.on('connecting', () => {
		start('2', 'CONNECT')
	})
	Fan.on('open', () => {
